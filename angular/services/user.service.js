export class UserService
{
	...
	{
		this.API.one( 'users', 'me' ).get().then( () => {
			this.$sendNewPassword( 'profile/password', data, success );
		}, () => {
			this.$sendNewPassword( 'password/' + data.token, data, success );
		} );
	}

	//
	$sendNewPassword( url, data, success )
	{
		this.API.all( url ).post( data ).then( ( response ) =>
		{
